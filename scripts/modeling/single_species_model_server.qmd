...
values(test_rast) <- runif(100)

terra::writeRaster(
  test_rast,
  filename = "/capstone/coastalconservation/data/processed/species_model_rasters/change_species_rasters/test.tif",
  overwrite = TRUE
)
```
...
**Aplysia californica** Worked, but only without GBM 

*Bugula neritina*

*Chondria arcuata* 

**Chondria dasyphylla** try without gbm

*Dictyota coriacea*

*Gastroclonium parvum* 

**Haminoea vesicula** try without gbm
...
Cirolana spp/Gnorimosphaeroma oregonense
...
single_species_model('Zonaria farlowii')
```
...
tmap_habitat_change('Norrisia norrisii')
```
