...
```
```{r}
alaria_obs_pa <- alaria_obs %>%
  mutate(status = if_else(num_count >= 1,
                          "presence",
                          "absent"))
```
...
https://cran.r-project.org/web/packages/SSDM/vignettes/SSDM.html#Summary_plot
...
```{r}
single_species_model('Littorina plena/scutulata')
```
