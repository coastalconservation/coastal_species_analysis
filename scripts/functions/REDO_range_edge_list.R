...
analyze_coastline_ranges <- function(cbs_excel_name = 'cbs_data_2025.xlsx',
  ...
  source(here::here('scripts', 'functions', 'REDO_species_ranges.R'))
  ...
  range_list <- map_dfr(1:(length(coastline_lat) - 1), function(i) {
    # 
    REDO_species_ranges(biodiv_total, coastline_lat[i], coastline_lat[i + 1]) %>%
  ...
  write.csv(range_list, here::here("data",
                                   "processed",
                                   "REDO_range_list.csv"), row.names = FALSE)
